...
void HTTPRoutes::initialize(WebServer& server) {
    ...
    handleIndexFile = std::bind(&HTTPRoutes::_handleIndexFile);
    handleBTScanner = std::bind(&HTTPRoutes::_handleBTScanner);
    handleLittleFSFile = std::bind(&HTTPRoutes::_handleLittleFSFile);
    handleConfigJSON = std::bind(&HTTPRoutes::_handleConfigJSON);
    handleRuntimeConfigJSON = std::bind(&HTTPRoutes::_handleRuntimeConfigJSON);
    handlePWCJSON = std::bind(&HTTPRoutes::_handlePWCJSON);
    handleShift = std::bind(&HTTPRoutes::_handleShift);
    handleHRSlider = std::bind(&HTTPRoutes::_handleHRSlider);
    handleWattsSlider = std::bind(&HTTPRoutes::_handleWattsSlider);
    handleCadSlider = std::bind(&HTTPRoutes::_handleCadSlider);
    handleERGMode = std::bind(&HTTPRoutes::_handleERGMode);
    handleTargetWattsSlider = std::bind(&HTTPRoutes::_handleTargetWattsSlider);
    handleLogin = std::bind(&HTTPRoutes::_handleLogin);
    handleOTAUpdate = std::bind(&HTTPRoutes::_handleOTAUpdate);
    handleFileUpload = std::bind(&HTTPRoutes::_handleFileUpload);
}
...
void HTTPRoutes::_handleFileUpload() {
    HTTPUpload& upload = currentServer->upload();
