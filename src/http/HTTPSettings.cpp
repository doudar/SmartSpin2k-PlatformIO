...
void HTTPSettings::processStepperSettings(WebServer& server) {
...
void HTTPSettings::sendSettingsResponse(WebServer& server, bool wasBTUpdate, bool wasSettingsUpdate, bool requiresReboot) {
    ...
    if (requiresReboot) {
        response = buildRedirectResponse(
            "Please wait while your settings are saved and SmartSpin2k reboots.",
            "/bluetoothscanner.html",
            5000
        );
    }
...
String HTTPSettings::buildRedirectResponse(const String& message, const String& page, int delay) {
    return "<!DOCTYPE html><html><body><h2>" + message + 
           "</h2></body><script>setTimeout(\"location.href = '" + page + 
           "';\", " + String(delay) + ");</script></html>";
}
